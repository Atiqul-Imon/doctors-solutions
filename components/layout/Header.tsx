...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 will-change-transform">
      {/* Top Bar - Fixed height to prevent flickering */}
      <div 
        className={`bg-gradient-to-r from-primary-50 to-primary-100/50 border-b border-primary-100 transition-all duration-300 ease-in-out overflow-hidden ${
      ...
      {/* Main Navigation - Fixed height and backdrop blur */}
      <nav className={`bg-white/95 backdrop-blur-md border-b transition-all duration-300 ease-in-out ${
        isScrolled 
          ...
          {isMenuOpen && (
            <div className="lg:hidden mt-4 pb-4 border-t border-gray-100 animate-fadeIn">
              <div className="flex flex-col space-y-2 pt-4">
                ...
                <Link
                  href="/login"
                  className="text-primary-600 px-4 py-3 rounded-lg font-medium text-center border border-primary-600 mt-2 hover:bg-primary-50 transition-colors"
                  onClick={() => setIsMenuOpen(false)}
                >
                  Login
                </Link>
